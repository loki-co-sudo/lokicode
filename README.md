...
- **計画→実行の引き渡し**：分析パイプライン自体は読み取り専用。**実行を伴う依頼（ファイル編集・コマンド・git commit/push 等）と判定した場合**は、最終回答（計画）を本物の Agent 実行パスに引き渡し、承認付きで**実際に**変更・コマンドを実行して結果を報告。実行タスクと判定したときは分析を自動で軽量化して高速化（重い多サンプルの分析を省く）。最終合成は「やった体」で書かず、実行していない作業は手順として提示
