...
- **アンサンブル（Mixture-of-Agents）**：難しめの課題では、ドラフトを廉価モデルで**並列複数生成→統合**し、最終回答も**複数候補を生成→強モデルが検証者として最良を選抜**。**トグルで ON/OFF 可能**——速度・コスト優先なら OFF（単発生成）、精度優先なら ON
- **速度の調整**：調査フェーズは**並列実行**（複数観点を同時に調べてレイテンシ短縮）。さらに「**検証の深さ**」を下げる／「**アンサンブル**」を OFF にすると、精度と引き換えに高速化できます
