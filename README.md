...
過去のロードマップ項目はすべて実装済みです（「主な機能」を参照）。新しいアイデアを随時追記します。

### ディープシンクの改善（AI 自体のコードを書いた）

**実装済み（0.46.0）**
- ✅ **証拠の厳密な選択**：意味目的な説明で埋めず絞り、各調査に役割と厳密なバジェットを割り当て（後半の長さを抑えつつ質を落とさない）
- ✅ **改善（REFINE）の強化**：低スコアならどのラウンドでもドラフト改善を強モデルに昇格（depth=1 でも弱モデル放置にしない）
- ✅ **判定出力の頑健性**：parseJudgment の解析失敗時、再用不能な構造の生出力を defect として流し、REFINE に自動的な指針を与える
- ✅ **ブリーフ解析の頑健性**：parseBrief がセクション区切りの表記揺れ（OBJECTIVE/目標、成功基準、調査 など）に対応

**残りの課題**
- **構造化された証拠**：findings にソース/確度タグを付け、「どの問いの答えか」「検証済みか未検証か」を保持
- **十分性ゲートの再帰化**：不足ギャップへの追加調査を 1 パスでなく再帰的に
- **難易度ルーターとの連動**：アンサンブル/幅/深さ/キャッシュルーターの難易度判定に連動
- **中断の洗練化**：API/ツール実行の途中までの中断と再開

### その他（検討中）

- （検討）UI の多言語対応（i18n）? 日本語前提のため優先度は中
- （検討）セッション管理の意味的改善、update_plan の plan->execute->verify ステートマシン
...
- **AI インラインコード補完（Copilot 風）** — カーソル位置に Ghost Text でリアルタイム補完を表示。Tab で受け入れ。コメントや周囲のコードから意図を読んで提案。OpenRouter の低レイテンシモデルを活用し、キー入力ごとにバックグラウンド推論。
...
- **スニペット管理** — AI が生成したコードや自作スニペットをライブラリ化。タグ付け・全文検索・ワンクリック挿入。.lokicode/snippets に保存し Git 管理可能に。AI にもスニペット検索ツールを提供。

#### プラットフォーム拡大

- **macOS / Linux 対応** — Tauri v2 のクロスプラットフォーム性を活かし、macOS（.dmg）と Linux（.deb / .AppImage）のビルド・配布に対応。CI に macOS と Linux ランナーを追加。
- **リモート開発（SSH / WSL）** — リモートサーバーや WSL ディストリビューション内のファイルシステムをマウントし、ローカルと同じ操作感で編集・AI エージェント実行。AI がリモート環境のシェルを直接操作して調査・変更可能に。

#### カスタマイズと拡張

- **カスタムツール定義** — ユーザーが AI エージェントに追加ツールを定義可能に。.lokicode/tools/*.yml にコマンド・説明・引数・承認レベルを記述するだけで、AI がそのツールを認識・使用できる。よく使うスクリプトや社内 API の呼び出しなどを登録。
...
- **オフラインモード** — API キー不要・完全ローカルで動作するモード。ローカル LLM のみでエージェント・ディープシンクを完結。機密コードも一切外部送信せず、エアギャップ環境でも AI 支援を利用可能に。
