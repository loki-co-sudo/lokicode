...
### ディープ推論の改善候補（AI 自身のコード分析より）

- **証拠の賢い選別**：証拠ブラックボードの単純な 9000 字クリップで大規模調査の重要情報が欠落しうる → 重要度スコア付き選別／要約圧縮へ
- **構造化された証拠**：findings を結合すると「どの調査がどの問いの答えか」「検証済みか仮説か」の区別が消える → ソース/確度タグ付きで保持
- **改善（REFINE）の強化**：検証は強モデル化済みだが修正は基本安モデルのまま → 低スコア継続時はドラフト改善も強モデルへ早期昇格
- **検証出力の頑健化**：`parseJudgment` が解析失敗時にスコア 60・汎用 defect へ楽観フォールバック → 構造化出力の強制／再試行
- **十分性ゲートの再帰化**：現状は不足ギャップへの追加調査が 1 パス・最大 3 件 → 新たに判明したギャップを再帰的に深掘り
- **難易度ルーターとの連動**：アンサンブル/深さ/広さを `breadth>1||depth>=3` の固定閾値でなく、ルーターの難易度判定に連動させる
- **ブリーフ解析の頑健化**：`parseBrief` が英語セクション名（GOAL/CRITERIA…）依存 → 表記揺れ・欠落に強く
- **中断の即応性**：停止ボタンが API／ツール実行の途中までは即時反映されない → より細かいアボート
