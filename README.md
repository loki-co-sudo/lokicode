...
**実装済み（0.47.0）**
- ✅ **構造化された証拠**：各調査結果を `VERIFIED`（file:line 付きの確認済み事実）/`ASSUMPTIONS`（未確認の推測）/`UNKNOWN` に区分して出力。ドラフトは VERIFIED のみ事実として扱い、審査は ASSUMPTIONS/UNKNOWN を事実と断定した主張を欠陥として減点
- ✅ **十分性ゲートの再帰化**：不足ギャップを追加調査したあと再度十分性を判定し、解消されるまで（上限 2 ラウンド）繰り返す
- ✅ **難易度ルーターとの連動**：オート振り分けで「ディープ」と判定された難タスクは、手動スライダーに依らず自動で深さ・サンプル数・アンサンブルを引き上げる
- ✅ **中断の即応性**：実行 ID をバックエンドに渡し、停止ボタンで進行中の API 呼び出し（ストリーミング含む）を即座に中断（ステップ境界を待たない）
