...
### ディープシンク（オーケストレータ型）
単発の API 呼び出しではなく、**設計ブリーフ（意図・成功基準・調査計画）→ 並列の接地調査 → 十分性ゲート → 証拠ベースのドラフト → 基準で採点する検証×N（合格で早期終了・低スコアは強モデルへ昇格）→ 最終合成**を行います。詳細は [パイプライン仕様 v2](specs/deep-reasoning-v2.md) / [エージェント設計 v3](specs/agent-v3.md) / [基盤設計](specs/architecture.md)。
