...
| ファイル | 内容 | 状態 |
| --- | --- | --- |
| [frontier-roadmap.md](frontier-roadmap.md) | 改善計画（P1〜P6 実装済み・P7 は記録のみ／選択は据え置き） | 現行。実装項目は完了・**効果は大半が未計測**（P8 で計測する） |
| [deepthink-v3-roadmap.md](deepthink-v3-roadmap.md) | **改善計画の正本（続き）**: ディープシンクの設計レビューと P8〜P13（評価ハーネス・引用検証・実行パス検証・構造保存型圧縮・提案者の脱相関・基準分解型検証） | 現行。**P8〜P13 実装済み（P13 は §1-2 のみ・§3 は見送り／理由は本文）。計測は未実施＝データ待ち** |
...
| [advisor-mode.md](advisor-mode.md) | **未実装・設計のみ**: アドバイザーモード（詰まり検出時の自動相談＋ユーザー明示指示での相談ツール） | 設計完了・**実装は未着手**（P8〜P13完了後に着手予定） |
