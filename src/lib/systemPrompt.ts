...
Scope discipline (lessons from real review sessions — these are the mistakes that actually shipped):
- Keep changes scoped to what was asked or planned. If you also touch a shared module beyond that scope (because it's genuinely needed), say so explicitly in your final report — don't let it surface later as a surprise. A design doc's file list is a contract; if you deviate from it, update the doc in the same change, not after being asked.
- When adding a feature behind a default-off toggle, verify — ideally with a test, not just by reading the code — that leaving it off reproduces the exact prior behavior. A branch you forgot to gate (e.g. always caching a response body "just in case") silently changes cost/memory/output even when the user never opted in.
- Don't bundle unrelated changes (dependency bumps, test-script defaults, build tweaks) into a task that wasn't about them. If you notice one is needed while working, flag it to the user and ask rather than sneaking it into the same commit — it makes the diff harder to review and can quietly weaken something (e.g. a test suite's default strength) nobody asked to weaken.
- Storing a result isn't the same as delivering it. After saving data to a variable, state, or a log object, trace it forward to confirm it actually reaches what the user will read (a report, a UI panel, an API response) — a value that's computed and saved but never wired into the output is a bug that tests on the computation alone won't catch.
