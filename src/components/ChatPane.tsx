...
const ChatPane = forwardRef<ChatPaneHandle, ChatPaneProps>(function ChatPane(
  ...
  // Global instructions loaded once from <home>/.lokicode/rules(.md) — apply
  // across every project, the way ~/.claude/CLAUDE.md does for Claude Code.
  const [globalRulesText, setGlobalRulesText] = useState("");
  // The two candidate absolute paths, kept as a narrow exact-path allowlist so
  // the agent can read/write ITS global rules file (only that file — never the
  // rest of ~/.lokicode, which also holds the updater signing key) even while
  // "restrict to workspace" stays on. Set once home is known, independent of
  // whether the file exists yet (write_file may be creating it for the first time).
  const [globalRulesPaths, setGlobalRulesPaths] = useState<string[]>([]);
  useEffect(() => {
    let cancelled = false;
    (async () => {
      let home: string;
      ...
      const candidates = [".lokicode/rules", ".lokicode/rules.md"].map((rel) =>
        joinPath(home, rel),
      );
      if (!cancelled) setGlobalRulesPaths(candidates);
      for (const path of candidates) {
        try {
          const txt = await invoke<string>("read_text_file", { path });
          if (!cancelled) {
            setGlobalRulesText(txt);
            return;
          }
        } catch {
          /* try next candidate */
      ...
      if (!cancelled) setGlobalRulesText("");
    })();
    return () => {
      cancelled = true;
    };
  ...
  const promptTokens = useMemo(() => {
    let tok = 180; // system prompt overhead (the fixed instruction block only —
    // rules text is counted separately below since its length varies per user)
    ...
    tok += approxTokens(rulesText) + approxTokens(globalRulesText);
    return tok;
  }, [items, input, includeFile, currentCode, rulesText, globalRulesText]);
      ...
      if (useDeep) {
        await runRecurrentReasoning(
          base,
          {
            ...
            extraAllowedPaths: globalRulesPaths,
